...
RetStatus SegmentVerifier::Verify()
{
    ...
    if (segmentMetaPage == nullptr) {
        ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::SEG_PAGE_READ_FAILED, m_segmentMetaPageId,
            "segment_meta_read_failed", 1, 0,
    ...
    *totalBlocks += segmentMetaPage->GetSelfExtentSize();
    ...
    if (m_options.checkExtentChain) {
        ...
        if (segmentMetaPage->GetSelfExtentSize() != expectedFirstSize) {
            ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::SEG_EXT_SIZE_INVALID,
                segmentMetaPage->GetSelfPageId(), "extent_size_invalid", expectedFirstSize,
                segmentMetaPage->GetSelfExtentSize(),
    ...
    while (currentExtentId.IsValid()) {
        if (!m_context->VisitPage(currentExtentId)) {
            ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::SEG_EXTENT_CHAIN_CYCLE, currentExtentId,
                "extent_chain_cycle", 1, 0,
        ...
        if (extentPage == nullptr) {
            ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::SEG_EXTENT_CHAIN_BROKEN, currentExtentId,
                "extent_chain_broken", 1, 0,
        ...
        if (extentPage->extentMeta.magic != EXTENT_META_MAGIC) {
            ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::SEG_MAGIC_MISMATCH, currentExtentId,
                "extent_magic_invalid", EXTENT_META_MAGIC, extentPage->extentMeta.magic,
                "Extent (%hu,%u) magic is invalid", currentExtentId.m_fileId, currentExtentId.m_blockId);
            m_pageSource->ReleasePage(extentBuffer);
            return DSTORE_FAIL;
        }
        ...
        if (actualSize != expectedSize) {
            ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::SEG_EXT_SIZE_INVALID, currentExtentId,
                "extent_size_invalid", expectedSize, actualSize,
    ...
    SegmentMetaPage *segmentMetaPage, const std::vector<ExtentInfo> &extents, uint64 walkedTotalBlocks)
{
    if (segmentMetaPage->extentMeta.magic != SEGMENT_META_MAGIC) {
        ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::SEG_MAGIC_MISMATCH,
            segmentMetaPage->GetSelfPageId(), "segment_magic_invalid", SEGMENT_META_MAGIC,
            segmentMetaPage->extentMeta.magic, "Segment meta page magic is invalid");
        return DSTORE_FAIL;
    }
    ...
    if (!IsDataSegmentType(segmentType) && segmentType != SegmentType::UNDO_SEGMENT_TYPE) {
        ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::SEG_SEGMENT_TYPE_INVALID,
            segmentMetaPage->GetSelfPageId(), "segment_type_invalid", 1, static_cast<uint64>(segmentType),
            "Segment (%hu,%u) has unsupported type %u", segmentMetaPage->GetFileId(),
    ...
    if (m_options.checkPageCounts && segmentMetaPage->GetTotalBlockCount() != walkedTotalBlocks) {
        ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::SEG_BLOCK_COUNT_MISMATCH,
            segmentMetaPage->GetSelfPageId(), "block_count_mismatch", segmentMetaPage->GetTotalBlockCount(),
            walkedTotalBlocks,
    ...
    if (segmentMetaPage->GetExtentCount() != extents.size()) {
        ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::SEG_EXTENT_COUNT_MISMATCH,
            segmentMetaPage->GetSelfPageId(), "extent_count_mismatch", segmentMetaPage->GetExtentCount(),
            extents.size(),
    ...
    if (IsDataSegmentType(segmentType)) {
        ...
        if ((dataMetaPage->GetDataBlockCount() == 0) != rangeUnset) {
            ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::SEG_DATA_RANGE_MISMATCH,
                segmentMetaPage->GetSelfPageId(), "data_range_mismatch",
        ...
        if (!rangeUnset &&
            ...
            ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::SEG_DATA_RANGE_MISMATCH,
                segmentMetaPage->GetSelfPageId(), "data_range_mismatch", 1, 0,
...
RetStatus SegmentVerifier::VerifyExtentBitmapConsistency(const std::vector<ExtentInfo> &extents)
{
    ...
    for (const ExtentInfo &extent : extents) {
        ...
        if (bitmapMetaPage == nullptr) {
            ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::SEG_PAGE_READ_FAILED, extent.pageId,
                "bitmap_meta_read_failed", 1, 0,
        ...
        if (!LocateBitmapBit(bitmapMetaPage, extent.pageId, &bitmapPageId, &bitNo)) {
            ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::BITMAP_NOT_SET, extent.pageId,
                "extent_not_in_bitmap", 1, 0,
        ...
        if (validatedBitmapPages.insert(bitmapKey).second) {
            ...
            if (popCount != bitmapPage->allocatedExtentCount) {
                ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::BITMAP_ALLOCATED_CNT_MISMATCH,
                    bitmapPageId, "bitmap_count_mismatch", bitmapPage->allocatedExtentCount, popCount,
    ...
    for (size_t i = 1; i < ranges.size(); ++i) {
        if (ranges[i - 1].second >= ranges[i].first) {
            ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::SEG_EXTENT_OVERLAP, m_segmentMetaPageId,
                "extent_overlap", ranges[i - 1].second, ranges[i].first,
                "Segment (%hu,%u) has overlapping extent ranges [%lu,%lu] and [%lu,%lu]",
    ...
    SegmentMetaPage *segmentMetaPage, const std::vector<ExtentInfo> &extents)
{
    ...
    if (!m_pageSource->GetIndexRootInfo(segmentMetaPage->GetSelfPageId(), &rootPageId, &rootLevel)) {
        ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::BTR_META_ROOT_INVALID,
            segmentMetaPage->GetSelfPageId(), "btree_root_missing", 1, 0,
    ...
    if (actualLeafCount != expectedLeafCount) {
        ReportResult(VerifySeverity::SEVERITY_WARNING, VerifyCode::SEG_LEAF_COUNT_MISMATCH,
            segmentMetaPage->GetSelfPageId(), "leaf_page_count_mismatch", expectedLeafCount, actualLeafCount,
...
RetStatus SegmentVerifier::CountLeafPagesBySiblingTraversal(const PageId &rootPageId, uint32 rootLevel, uint64 *leafCount)
{
    ...
    if (!currentPageId.IsValid()) {
        ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::BTR_META_ROOT_INVALID, rootPageId,
            "leaf_level_resolution_failed", rootLevel, 0,
    ...
    *leafCount = 0;
    while (currentPageId.IsValid()) {
        ...
        if (!page->GetLinkAndStatus()->TestType(BtrPageType::LEAF_PAGE) || page->GetLinkAndStatus()->GetLevel() != 0) {
            ReportResult(VerifySeverity::SEVERITY_ERROR, VerifyCode::BTR_LEVEL_INCONSISTENT, currentPageId,
                "leaf_page_level_invalid", 0, page->GetLinkAndStatus()->GetLevel(),
...
void SegmentVerifier::ReportResult(VerifySeverity severity, VerifyCode code, const PageId &pageId,
